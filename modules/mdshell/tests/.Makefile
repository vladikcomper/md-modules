...
hello-world:	| $(TEST_BUILD_DIR) $(CONVSYM)
	$(ASM68K) /q /k /m /o c+,ws+,op+,os+,ow+,oz+,oaq+,osq+,omq+,ae- /p hello-world.asm, $(TEST_BUILD_DIR)/hello-world.gen, $(TEST_BUILD_DIR)/hello-world.sym, $(TEST_BUILD_DIR)/hello-world.lst
	$(CONVSYM) $(TEST_BUILD_DIR)/hello-world.sym $(TEST_BUILD_DIR)/hello-world.gen -a -ref 200 -debug


raise-error:	| $(TEST_BUILD_DIR) $(CONVSYM)
	$(ASM68K) /q /k /m /o c+,ws+,op+,os+,ow+,oz+,oaq+,osq+,omq+,ae- /p raise-error.asm, $(TEST_BUILD_DIR)/raise-error.gen, $(TEST_BUILD_DIR)/raise-error.sym, $(TEST_BUILD_DIR)/raise-error.lst
...
linkable:	| $(TEST_BUILD_DIR) $(MDSHELL_HEADLESS) $(CONVSYM)
	$(ASM68K) /q /k /m /o c+,ws+,op+,os+,ow+,oz+,oaq+,osq+,omq+,ae- /g /l linkable.asm, $(TEST_BUILD_DIR)/linkable.obj, , $(TEST_BUILD_DIR)/linkable.lst
	$(PSYLINK) /q /p $(BUILD_DIR)/asm68k-linkable/MDShell.obj $(TEST_BUILD_DIR)/linkable.obj,$(TEST_BUILD_DIR)/linkable.gen,$(TEST_BUILD_DIR)/linkable.sym
...
flow-test:	| $(TEST_BUILD_DIR) $(CONVSYM) $(CBUNDLE)
	...
	$(ASM68K) /q /k /m /o c+,ws+,op+,os+,ow+,oz+,oaq+,osq+,omq+,ae-,l+ /p $(TEST_BUILD_DIR)/flow-test-asm68k.asm, $(TEST_BUILD_DIR)/flow-test-asm68k.gen, $(TEST_BUILD_DIR)/flow-test-asm68k.sym, $(TEST_BUILD_DIR)/flow-test-asm68k.lst
