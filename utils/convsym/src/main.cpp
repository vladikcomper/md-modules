...
int main (int argc, const char ** argv) {
	...
	if (argc < 3) {
		std::fputs(
			...
			"      -in asm68k_lst -inopt \"/localSign=@ /localJoin=. /ignoreMacroDefs+ /ignoreMacroExp- /addMacrosAsOpcodes+ /processLocals+ /strictSymbolOrder-\"\n"
			"      -in as_lst -inopt \"/localJoin=. /processLocals+ /ignoreInternalSymbols+\"\n"
			"      -in as_lst_exp -inopt \"/strictSymbolOrder-\"\n"
